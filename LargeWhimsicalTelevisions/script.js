...
window.onload = function () {
  //rank function
  // const moviesM = sortMoviesByRank(movies);
  userInput = prompt("Do you want to order by title, rank or ID?");
  let sortAtt;
  if (userInput === null) {
    sortAtt = sortMoviesByAttr(movies, "rank");
  } else {
    sortAtt = sortMoviesByAttr(movies, userInput);
  }
  console.log(userInput);
  // Display Movies list
  displayMovies(sortAtt);
};
...
function sortMoviesByAttr(movies, sortAttr) {
  ...
  for (let j = 0; j < movies.length - 1; j++) {
    var max_num;
    if (userInput === null) {
      max_num = getMaxMovieObject(movies, j, "rank");
    } else {
      max_num = getMaxMovieObject(movies, j, userInput);
    }
    ...
    // }
    // swap the first and the last
    movies[max_num["max_index"]] = movies[j]; // --> 10
    movies[j] = max_num["max_number"];
  }
